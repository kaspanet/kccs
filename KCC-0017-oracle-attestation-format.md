...
| signature | 64 | SECP256k1 signature over bytes 0-104 |
...
5. **Operator check**: `operator_id` must be ACTIVE in the Oracle Registry (per KCC-0018)
...
6. Operator ID must be the compressed SECP256k1 public key (33 bytes, using the full 33-byte compressed encoding including the parity prefix byte (0x02 or 0x03)).
7. Signature covers bytes 0-104 (everything before the signature field).